// sbgemm_amx_v1.2.0.c - Intel AMX最適化版BFloat16 GEMM
// PG1.1: SIMD最適化担当
// v1.2.0: パッキング改善 + Bタイル再利用最適化 + ループ順序改善
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
// AVX-512でAをパッキング（改善版：ストリーミングストア）
        ...
        for (int kt = 0; kt < k_tiles; kt++) {
            ...
            if (kt + 1 < k_tiles) {
                for (int ii = 0; ii < TILE_M; ii += 4) {
                    _mm_prefetch((const char*)(A + (m_base + ii) * lda + k_base + TILE_K), _MM_HINT_T0);
                }
            }

            for (int ii = 0; ii < TILE_M; ii++) {
                int m_idx = m_base + ii;
                if (m_idx < M && k_base + TILE_K <= k_start + k_block) {
...
static void pack_B_avx512(const bf16 *B, bf16 *buffer, int K, int N, int ldb,
    ...
    const int STRIDE = TILE_N * 2;  // インターリーブ後の行ストライド

    for (int nt = 0; nt < n_tiles; nt++) {
        int n_base = n_start + nt * TILE_N;

        for (int kt = 0; kt < k_tiles; kt++) {
            bf16 *dest = buffer + (nt * k_tiles + kt) * (TILE_K / 2) * STRIDE;
            int k_base = k_start + kt * TILE_K;

            // プリフェッチ次のKブロック
            if (kt + 1 < k_tiles) {
                _mm_prefetch((const char*)(B + (k_base + TILE_K) * ldb + n_base), _MM_HINT_T0);
                _mm_prefetch((const char*)(B + (k_base + TILE_K + 1) * ldb + n_base), _MM_HINT_T0);
            }

            // インターリーブパッキング：k0,k1のペアをN方向に並べる
            for (int kk = 0; kk < TILE_K / 2; kk++) {
                int k_idx0 = k_base + kk * 2;
                int k_idx1 = k_base + kk * 2 + 1;

                if (k_idx1 < k_start + k_block && n_base + TILE_N <= n_start + n_block) {
                    // フルタイル：AVX-512で2行を同時処理
                    __m256i row0 = _mm256_loadu_si256((const __m256i*)(B + k_idx0 * ldb + n_base));
                    __m256i row1 = _mm256_loadu_si256((const __m256i*)(B + k_idx1 * ldb + n_base));

                    // インターリーブ: [a0,a1,a2...] + [b0,b1,b2...] -> [a0,b0,a1,b1,...]
                    __m256i lo = _mm256_unpacklo_epi16(row0, row1);
                    __m256i hi = _mm256_unpackhi_epi16(row0, row1);

                    // 256ビットレーン間のシャッフル
                    __m256i perm0 = _mm256_permute2x128_si256(lo, hi, 0x20);
                    __m256i perm1 = _mm256_permute2x128_si256(lo, hi, 0x31);

                    _mm256_storeu_si256((__m256i*)(dest + kk * STRIDE), perm0);
                    _mm256_storeu_si256((__m256i*)(dest + kk * STRIDE + 16), perm1);
                } else {
                    // 境界処理：スカラーフォールバック
                    for (int nn = 0; nn < TILE_N; nn++) {
                        int n_idx = n_base + nn;
                        if (n_idx < n_start + n_block) {
                            dest[kk * STRIDE + nn * 2] = (k_idx0 < k_start + k_block) ? B[k_idx0 * ldb + n_idx] : 0;
                            dest[kk * STRIDE + nn * 2 + 1] = (k_idx1 < k_start + k_block) ? B[k_idx1 * ldb + n_idx] : 0;
                        } else {
                            dest[kk * STRIDE + nn * 2] = 0;
                            dest[kk * STRIDE + nn * 2 + 1] = 0;
                        }
                    }
                }
            }
...
static void amx_kernel_tiling_b(const bf16 *A_pack, const bf16 *B_pack, float *C,
    ...
    int m_tiles = (M + TILE_M - 1) / TILE_M;
    const int B_TILE_SIZE = (TILE_K / 2) * (TILE_N * 2);

    // ループ順序変更: N->M->K (Bの再利用を最大化)
    for (int nt = 0; nt < n_tiles; nt += 3) {
        ...
        for (int mt = 0; mt < m_tiles; mt += 2) {
            ...
            for (int ii = 0; ii < TILE_M; ii++) {
                if (m0 + ii < M) {
                    if (n0 + TILE_N <= n_start + n_block) {
                        __m512 cv = _mm512_loadu_ps(C + (m0 + ii) * ldc + n0);
                        _mm512_store_ps(C00 + ii * TILE_N, cv);
                    } else {
                        for (int jj = 0; jj < TILE_N; jj++)
                            C00[ii * TILE_N + jj] = (n0 + jj < n_start + n_block) ? C[(m0 + ii) * ldc + n0 + jj] : 0.0f;
                    }
                    if (n_valid1) {
                        if (n1 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_loadu_ps(C + (m0 + ii) * ldc + n1);
                            _mm512_store_ps(C01 + ii * TILE_N, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                C01[ii * TILE_N + jj] = (n1 + jj < n_start + n_block) ? C[(m0 + ii) * ldc + n1 + jj] : 0.0f;
                        }
                    } else {
                        memset(C01 + ii * TILE_N, 0, TILE_N * sizeof(float));
                    }
                    if (n_valid2) {
                        if (n2 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_loadu_ps(C + (m0 + ii) * ldc + n2);
                            _mm512_store_ps(C02 + ii * TILE_N, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                C02[ii * TILE_N + jj] = (n2 + jj < n_start + n_block) ? C[(m0 + ii) * ldc + n2 + jj] : 0.0f;
                        }
                    } else {
                        memset(C02 + ii * TILE_N, 0, TILE_N * sizeof(float));
                    }
                } else {
                    memset(C00 + ii * TILE_N, 0, TILE_N * sizeof(float));
                    memset(C01 + ii * TILE_N, 0, TILE_N * sizeof(float));
                    memset(C02 + ii * TILE_N, 0, TILE_N * sizeof(float));
                }
            }
            ...
            if (m_valid1) {
                for (int ii = 0; ii < TILE_M; ii++) {
                    if (m1 + ii < M) {
                        if (n0 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_loadu_ps(C + (m1 + ii) * ldc + n0);
                            _mm512_store_ps(C10 + ii * TILE_N, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                C10[ii * TILE_N + jj] = (n0 + jj < n_start + n_block) ? C[(m1 + ii) * ldc + n0 + jj] : 0.0f;
                        }
                        if (n_valid1) {
                            if (n1 + TILE_N <= n_start + n_block) {
                                __m512 cv = _mm512_loadu_ps(C + (m1 + ii) * ldc + n1);
                                _mm512_store_ps(C11 + ii * TILE_N, cv);
                            } else {
                                for (int jj = 0; jj < TILE_N; jj++)
                                    C11[ii * TILE_N + jj] = (n1 + jj < n_start + n_block) ? C[(m1 + ii) * ldc + n1 + jj] : 0.0f;
                            }
                        } else {
                            memset(C11 + ii * TILE_N, 0, TILE_N * sizeof(float));
                        }
                        if (n_valid2) {
                            if (n2 + TILE_N <= n_start + n_block) {
                                __m512 cv = _mm512_loadu_ps(C + (m1 + ii) * ldc + n2);
                                _mm512_store_ps(C12 + ii * TILE_N, cv);
                            } else {
                                for (int jj = 0; jj < TILE_N; jj++)
                                    C12[ii * TILE_N + jj] = (n2 + jj < n_start + n_block) ? C[(m1 + ii) * ldc + n2 + jj] : 0.0f;
                            }
                        } else {
                            memset(C12 + ii * TILE_N, 0, TILE_N * sizeof(float));
                        }
                    } else {
                        memset(C10 + ii * TILE_N, 0, TILE_N * sizeof(float));
                        memset(C11 + ii * TILE_N, 0, TILE_N * sizeof(float));
                        memset(C12 + ii * TILE_N, 0, TILE_N * sizeof(float));
                    }
                }
            ...
            for (int kt = 0; kt < k_tiles; kt++) {
                ...
                const bf16 *B0 = B_pack + (nt * k_tiles + kt) * B_TILE_SIZE;
                const bf16 *B1 = B_pack + ((nt + 1) * k_tiles + kt) * B_TILE_SIZE;
                const bf16 *B2 = B_pack + ((nt + 2) * k_tiles + kt) * B_TILE_SIZE;

                // 積極的プリフェッチ
                if (kt + 1 < k_tiles) {
                    _mm_prefetch((const char*)(A0 + TILE_M * TILE_K), _MM_HINT_T0);
                    _mm_prefetch((const char*)(A0 + TILE_M * TILE_K + 64), _MM_HINT_T0);
                    if (m_valid1) {
                        _mm_prefetch((const char*)(A1 + TILE_M * TILE_K), _MM_HINT_T0);
                    }
                    _mm_prefetch((const char*)(B0 + B_TILE_SIZE), _MM_HINT_T0);
                }
            ...
            for (int ii = 0; ii < TILE_M; ii++) {
                if (m0 + ii < M) {
                    if (n0 + TILE_N <= n_start + n_block) {
                        __m512 cv = _mm512_load_ps(C00 + ii * TILE_N);
                        _mm512_storeu_ps(C + (m0 + ii) * ldc + n0, cv);
                    } else {
                        for (int jj = 0; jj < TILE_N; jj++)
                            if (n0 + jj < n_start + n_block) C[(m0 + ii) * ldc + n0 + jj] = C00[ii * TILE_N + jj];
                    }
                    if (n_valid1) {
                        if (n1 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_load_ps(C01 + ii * TILE_N);
                            _mm512_storeu_ps(C + (m0 + ii) * ldc + n1, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                if (n1 + jj < n_start + n_block) C[(m0 + ii) * ldc + n1 + jj] = C01[ii * TILE_N + jj];
                        }
                    }
                    if (n_valid2) {
                        if (n2 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_load_ps(C02 + ii * TILE_N);
                            _mm512_storeu_ps(C + (m0 + ii) * ldc + n2, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                if (n2 + jj < n_start + n_block) C[(m0 + ii) * ldc + n2 + jj] = C02[ii * TILE_N + jj];
                        }
                    }
                }
            }
            if (m_valid1) {
                for (int ii = 0; ii < TILE_M; ii++) {
                    if (m1 + ii < M) {
                        if (n0 + TILE_N <= n_start + n_block) {
                            __m512 cv = _mm512_load_ps(C10 + ii * TILE_N);
                            _mm512_storeu_ps(C + (m1 + ii) * ldc + n0, cv);
                        } else {
                            for (int jj = 0; jj < TILE_N; jj++)
                                if (n0 + jj < n_start + n_block) C[(m1 + ii) * ldc + n0 + jj] = C10[ii * TILE_N + jj];
                        }
                        if (n_valid1) {
                            if (n1 + TILE_N <= n_start + n_block) {
                                __m512 cv = _mm512_load_ps(C11 + ii * TILE_N);
                                _mm512_storeu_ps(C + (m1 + ii) * ldc + n1, cv);
                            } else {
                                for (int jj = 0; jj < TILE_N; jj++)
                                    if (n1 + jj < n_start + n_block) C[(m1 + ii) * ldc + n1 + jj] = C11[ii * TILE_N + jj];
                            }
                        }
                        if (n_valid2) {
                            if (n2 + TILE_N <= n_start + n_block) {
                                __m512 cv = _mm512_load_ps(C12 + ii * TILE_N);
                                _mm512_storeu_ps(C + (m1 + ii) * ldc + n2, cv);
                            } else {
                                for (int jj = 0; jj < TILE_N; jj++)
                                    if (n2 + jj < n_start + n_block) C[(m1 + ii) * ldc + n2 + jj] = C12[ii * TILE_N + jj];
                            }
                        }
                    }
                }
...
int main(int argc, char *argv[]) {
    ...
    printf("Intel AMX BF16 GEMM Benchmark (v1.2.0 - Packing Optimized)\n");
