// sbgemm_amx_v1.0.1.c - Intel AMX最適化版BFloat16 GEMM
// PG1.1: SIMD最適化担当
// v1.0.1: タイルレジスタ番号を定数に修正
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
        ...
        for (int kt = 0; kt < k_tiles; kt++) {
            ...
            for (int ii = 0; ii < TILE_M; ii++) {
                for (int kk = 0; kk < TILE_K; kk++) {
                    int m_idx = m_base + ii;
                    int k_idx = k_base + kk;
                    if (m_idx < M && k_idx < k_start + k_block) {
                        dest[ii * TILE_K + kk] = A[m_idx * lda + k_idx];
                    } else {
                        dest[ii * TILE_K + kk] = 0;
                    }
                }
...
// 行列Bをパッキング（AMXインターリーブ形式）
static void pack_B(const bf16 *B, bf16 *buffer, int K, int N, int ldb,
            ...
            for (int kk = 0; kk < TILE_K / 2; kk++) {
                for (int nn = 0; nn < TILE_N; nn++) {
                    ...
                    if (n_idx < n_start + n_block && k_idx0 < k_start + k_block) {
                        ...
                        if (k_idx1 < k_start + k_block) {
                            dest[kk * TILE_N * 2 + nn * 2 + 1] = B[k_idx1 * ldb + n_idx];
                        } else {
...
// AMX 2x2カーネル（固定タイル番号）
static void amx_kernel_2x2(const bf16 *A_pack, const bf16 *B_pack, float *C,
                           int M, int n_block, int k_block, int ldc, int n_start,
                           int k_tiles, int n_tiles) {

    for (int mt = 0; mt < (M + TILE_M - 1) / TILE_M; mt += 2) {
        for (int nt = 0; nt < n_tiles; nt += 2) {
            ...
            for (int ii = 0; ii < TILE_M; ii++) {
                for (int jj = 0; jj < TILE_N; jj++) {
                    C00[ii * TILE_N + jj] = (m0 + ii < M && n0 + jj < n_start + n_block) ?
                                             C[(m0 + ii) * ldc + n0 + jj] : 0.0f;
                    C01[ii * TILE_N + jj] = (m0 + ii < M && n1 + jj < n_start + n_block && nt + 1 < n_tiles) ?
                                             C[(m0 + ii) * ldc + n1 + jj] : 0.0f;
                    C10[ii * TILE_N + jj] = (m1 + ii < M && n0 + jj < n_start + n_block && mt + 1 < (M + TILE_M - 1) / TILE_M) ?
                                             C[(m1 + ii) * ldc + n0 + jj] : 0.0f;
                    C11[ii * TILE_N + jj] = (m1 + ii < M && n1 + jj < n_start + n_block && mt + 1 < (M + TILE_M - 1) / TILE_M && nt + 1 < n_tiles) ?
                                             C[(m1 + ii) * ldc + n1 + jj] : 0.0f;
                }
            }
            ...
            _tile_loadd(2, C10, TILE_N * sizeof(float));
            _tile_loadd(3, C11, TILE_N * sizeof(float));
            ...
            _tile_stored(2, C10, TILE_N * sizeof(float));
            _tile_stored(3, C11, TILE_N * sizeof(float));

            // Cに書き戻し
            for (int ii = 0; ii < TILE_M; ii++) {
                for (int jj = 0; jj < TILE_N; jj++) {
                    if (m0 + ii < M && n0 + jj < n_start + n_block)
                        C[(m0 + ii) * ldc + n0 + jj] = C00[ii * TILE_N + jj];
                    if (m0 + ii < M && n1 + jj < n_start + n_block && nt + 1 < n_tiles)
                        C[(m0 + ii) * ldc + n1 + jj] = C01[ii * TILE_N + jj];
                    if (m1 + ii < M && n0 + jj < n_start + n_block && mt + 1 < (M + TILE_M - 1) / TILE_M)
                        C[(m1 + ii) * ldc + n0 + jj] = C10[ii * TILE_N + jj];
                    if (m1 + ii < M && n1 + jj < n_start + n_block && mt + 1 < (M + TILE_M - 1) / TILE_M && nt + 1 < n_tiles)
                        C[(m1 + ii) * ldc + n1 + jj] = C11[ii * TILE_N + jj];
                }
            }
        }
    }
...
void sbgemm_nolib(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB,
    ...
    if (init_amx() != 0) {
        fprintf(stderr, "AMX init failed, using scalar.\n");
        sbgemm_scalar(M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
        return;
    }
    ...
    for (int k_start = 0; k_start < K; k_start += BLOCK_K) {
        ...
        for (int n_start = 0; n_start < N; n_start += BLOCK_N) {
            ...
            pack_B(B, buffer_B, K, N, ldb, k_start, k_block, n_start, n_block);
            amx_kernel_2x2(buffer_A, buffer_B, C, M, n_block, k_block, ldc, n_start, k_tiles, n_tiles);
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("Intel AMX BF16 GEMM Benchmark (v1.0.1)\n");
    ...
    for (int s = 0; s < num_sizes; s++) {
        ...
        if (!A || !B || !C) {
            printf("Memory alloc failed for size %d\n", M);
