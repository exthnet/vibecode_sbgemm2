// sbgemm_amx_v1.3.0.c - Intel AMX最適化版BFloat16 GEMM
// PG1.1: SIMD最適化担当
// v1.3.0: プリフェッチ距離最適化 + シンプルパッキング + Kループアンロール
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
#define BLOCK_K 1536
#define BLOCK_N 480
#define PREFETCH_DIST 2  // プリフェッチ距離: 2タイル先
...
static void pack_A_avx512(const bf16 *A, bf16 *buffer, int M, int K, int lda, int k_start, int k_block) {
    ...
    for (int mt = 0; mt < m_tiles; mt++) {
        ...
        for (int kt = 0; kt < k_tiles; kt++) {
            ...
            // L2へのプリフェッチ（2タイル先）
            if (kt + PREFETCH_DIST < k_tiles) {
                _mm_prefetch((const char*)(A + m_base * lda + k_base + TILE_K * PREFETCH_DIST), _MM_HINT_T1);
            }
...
// Bをパッキング（シンプル版、L2プリフェッチ追加）
static void pack_B_simple(const bf16 *B, bf16 *buffer, int K, int N, int ldb,
    ...
    for (int nt = 0; nt < n_tiles; nt++) {
        ...
        for (int kt = 0; kt < k_tiles; kt++) {
            ...
            // L2プリフェッチ
            if (kt + PREFETCH_DIST < k_tiles) {
                _mm_prefetch((const char*)(B + (k_base + TILE_K * PREFETCH_DIST) * ldb + n_base), _MM_HINT_T1);
            }
...
static void amx_kernel_tiling_b(const bf16 *A_pack, const bf16 *B_pack, float *C,
    ...
    const int A_TILE_SIZE = TILE_M * TILE_K;
    ...
    for (int mt = 0; mt < m_tiles; mt += 2) {
        ...
        for (int nt = 0; nt < n_tiles; nt += 3) {
            ...
            // Kループ（2イテレーションずつアンロール）
            int kt = 0;
            for (; kt + 1 < k_tiles; kt += 2) {
                // イテレーション0
                const bf16 *A0_0 = A_pack + (mt * k_tiles + kt) * A_TILE_SIZE;
                const bf16 *A1_0 = A_pack + ((mt + 1) * k_tiles + kt) * A_TILE_SIZE;
                const bf16 *B0_0 = B_pack + (nt * k_tiles + kt) * B_TILE_SIZE;
                const bf16 *B1_0 = B_pack + ((nt + 1) * k_tiles + kt) * B_TILE_SIZE;
                const bf16 *B2_0 = B_pack + ((nt + 2) * k_tiles + kt) * B_TILE_SIZE;

                // イテレーション1のアドレス計算とプリフェッチ
                const bf16 *A0_1 = A0_0 + A_TILE_SIZE;
                const bf16 *A1_1 = A1_0 + A_TILE_SIZE;
                const bf16 *B0_1 = B0_0 + B_TILE_SIZE;
                const bf16 *B1_1 = B1_0 + B_TILE_SIZE;
                const bf16 *B2_1 = B2_0 + B_TILE_SIZE;

                // 2イテレーション先をプリフェッチ
                if (kt + 2 < k_tiles) {
                    _mm_prefetch((const char*)(A0_0 + 2 * A_TILE_SIZE), _MM_HINT_T0);
                    _mm_prefetch((const char*)(B0_0 + 2 * B_TILE_SIZE), _MM_HINT_T0);
                }

                // --- イテレーション0 ---
                _tile_loadd(6, A0_0, TILE_K * sizeof(bf16));
                _tile_loadd(7, B0_0, TILE_N * 2 * sizeof(bf16));
                _tile_dpbf16ps(0, 6, 7);

                if (n_valid1) {
                    _tile_loadd(7, B1_0, TILE_N * 2 * sizeof(bf16));
                    _tile_dpbf16ps(1, 6, 7);
                }
                if (n_valid2) {
                    _tile_loadd(7, B2_0, TILE_N * 2 * sizeof(bf16));
                    _tile_dpbf16ps(2, 6, 7);
                }

                if (m_valid1) {
                    _tile_loadd(6, A1_0, TILE_K * sizeof(bf16));
                    _tile_loadd(7, B0_0, TILE_N * 2 * sizeof(bf16));
                ...
                // --- イテレーション1 ---
                _tile_loadd(6, A0_1, TILE_K * sizeof(bf16));
                _tile_loadd(7, B0_1, TILE_N * 2 * sizeof(bf16));
                _tile_dpbf16ps(0, 6, 7);

                if (n_valid1) {
                    _tile_loadd(7, B1_1, TILE_N * 2 * sizeof(bf16));
                    _tile_dpbf16ps(1, 6, 7);
                }
                if (n_valid2) {
                    _tile_loadd(7, B2_1, TILE_N * 2 * sizeof(bf16));
                    _tile_dpbf16ps(2, 6, 7);
                }

                if (m_valid1) {
                    _tile_loadd(6, A1_1, TILE_K * sizeof(bf16));
                    _tile_loadd(7, B0_1, TILE_N * 2 * sizeof(bf16));
                    _tile_dpbf16ps(3, 6, 7);
                    if (n_valid1) {
                        _tile_loadd(7, B1_1, TILE_N * 2 * sizeof(bf16));
            ...
            // 残りのイテレーション
            for (; kt < k_tiles; kt++) {
                const bf16 *A0 = A_pack + (mt * k_tiles + kt) * A_TILE_SIZE;
                const bf16 *A1 = A_pack + ((mt + 1) * k_tiles + kt) * A_TILE_SIZE;
...
void sbgemm_nolib(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB,
    ...
    for (int k_start = 0; k_start < K; k_start += BLOCK_K) {
        ...
        for (int n_start = 0; n_start < N; n_start += BLOCK_N) {
            ...
            pack_B_simple(B, buffer_B, K, N, ldb, k_start, k_block, n_start, n_block);
...
int main(int argc, char *argv[]) {
    ...
    printf("Intel AMX BF16 GEMM Benchmark (v1.3.0 - K-Unroll + L2 Prefetch)\n");
    printf("Block sizes: k=%d, n=%d, prefetch_dist=%d\n", BLOCK_K, BLOCK_N, PREFETCH_DIST);
