// sbgemm_v1.1.0.c - SIMD最適化版 BF16 GEMM (改良版)
// PG1.8 - Intel oneAPI 2025 / AVX-512
//
// v1.0.0からの改善:
// - プリフェッチ命令追加（L1キャッシュへの先読み）
// - ループアンローリング強化（K方向2段展開）
// - reference.pdf参考のブロックサイズ調整（k=1536に近づける）
...
/* --- キャッシュブロッキング定数 (v1.1.0: 調整済み) --- */
// reference.pdf参考: k=1536, n=480が最適
// L2キャッシュ(2MB)を考慮
#define BLOCK_M 64
#define BLOCK_N 64
#define BLOCK_K 512  // 大きめのKブロックでデータ再利用向上
...
/* --- SIMD最適化版 sbgemm (NoTrans, NoTrans専用) v1.1.0 --- */
void sbgemm_simd_nn_v11(int M, int N, int K,
                        float alpha,
                ...
                for (int i = ib; i < i_end; ++i) {
                    ...
                    for (; j + 16 <= j_end; j += 16) {
                        ...
                        for (; k + 2 <= k_end; k += 2) {
                            // プリフェッチ: 次のBブロック
                            if (k + 4 < k_end) {
                                PREFETCH_L1(&B[(k + 4) * ldb + j]);
                            }

                            // k
                            float a_val0 = bf16_to_float(A[i * lda + k]);
                            __m512 a_vec0 = _mm512_set1_ps(a_val0 * alpha);
                            __m256i b_bf16_0 = _mm256_loadu_si256(
                                (const __m256i*)&B[k * ldb + j]);
                            __m512 b_vec0 = bf16x16_to_fp32(b_bf16_0);
                            sum = _mm512_fmadd_ps(a_vec0, b_vec0, sum);

                            // k+1
                            float a_val1 = bf16_to_float(A[i * lda + k + 1]);
                            __m512 a_vec1 = _mm512_set1_ps(a_val1 * alpha);
                            __m256i b_bf16_1 = _mm256_loadu_si256(
                                (const __m256i*)&B[(k + 1) * ldb + j]);
                            __m512 b_vec1 = bf16x16_to_fp32(b_bf16_1);
                            sum = _mm512_fmadd_ps(a_vec1, b_vec1, sum);
                        }
                  ...
                  float *C, int ldc)
{
    ...
    if (transA == CblasNoTrans && transB == CblasNoTrans) {
        sbgemm_simd_nn_v11(M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
        return;
    }

    // スカラ版フォールバック（省略: v1.0.0と同じ）
    if (beta == 0.0f) {
        for (int i = 0; i < M; ++i) {
            for (int j = 0; j < N; ++j) {
                C[i * ldc + j] = 0.0f;
...
int main(int argc, char *argv[]) {
    ...
    printf("SBGEMM SIMD v1.1.0 Benchmark (oneAPI 2025)\n");
    printf("Improvements: Prefetch + K-unroll(2)\n");
